...
for i=1:m
    ...
    a3 = sigmoid(z3);
    h = a3;
    yTemp = y(i) == (1:num_labels)';
    for j=1:num_labels
        J = J-yTemp(j)*log(h(j))-(1-yTemp(j))*log(1-h(j));
    end
%     J = J+sum(-yTemp.*log(h)... %向量化之后循环起来反而更慢了，想不通
%         -(ones(num_labels,1)-yTemp).*log(ones(num_labels,1)-h));   
...
J = 1/m*J;
J = J+lambda/2/m*...
    (sum(Theta1(:,2:input_layer_size+1).^2,'all')+...
    sum(Theta2(:,2:hidden_layer_size+1).^2,'all'));
